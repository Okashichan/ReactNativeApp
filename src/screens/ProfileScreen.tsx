...
function ProfileScreen(): React.JSX.Element {
  ...
  const handleButtonPress = () => {
    Alert.alert(
      'Реєстрація',
      `Ім'я: ${user.name}\nПрізвище: ${user.surname}\nЕлектронна пошта: ${user.mail}\nПароль: ${user.password}\nПароль (ще раз): ${user.passwordRepeat}`,
      [
        {
          text: 'OK',
          onPress: () =>
            setUser({ mail: '', password: '', surname: '', name: '', passwordRepeat: '' }),
        },
        { text: 'Відміна' },
      ],
    );
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <LabeledInput
        ...
        secureTextEntry
        onChangeText={e => handleInputChange('passwordRepeat', e)}
      />
      <LabeledInput
        label="Прізвище"
        value={user.surname}
      ...
      <View style={styles.button}>
        <Button color="#ff80c0" title="Зареєструватися" onPress={handleButtonPress} />
      </View>
    </ScrollView>
  );
}
